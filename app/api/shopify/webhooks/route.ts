...
import {contactFromWebhook,upsertShopifyContact} from "@/lib/shopify-contacts";
...
export async function POST(request:NextRequest){
  ...
  try{
    const payload=JSON.parse(raw),topic=request.headers.get("x-shopify-topic")||"products/update",webhookId=request.headers.get("x-shopify-webhook-id");
    ...
    if(topic.startsWith("customers/")){
      await upsertShopifyContact(organization.id,contactFromWebhook(payload));
      await supabase.from("audit_logs").insert({organization_id:organization.id,action:topic.replace("/", "."),object_type:"contact",object_id:String(payload.id),metadata:{webhook_id:webhookId}});
