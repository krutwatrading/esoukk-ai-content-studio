...
export default function WhatsAppApprovalPanel({product,campaign,publishImage}:{product:ProductData;campaign:CampaignCopy;publishImage?:string}){
  ...
  const[body,setBody]=useState(campaign.whatsappBody||"");
  ...
  async function saveDraft(){
    ...
    try{
      ...
      const data=await call("POST",{platform:"whatsapp",product,imageUrl,templateName,templateLanguage:language,body,cta:campaign.whatsappCta||"VIEW PRODUCT"});
  ...
  return <section className="publishing-approval whatsapp-approval">
    ...
    <div className="publishing-actions">
      ...
      {(state==="approved"||state==="scheduling")&&<><label>UAE DATE & TIME<input type="datetime-local" value={scheduledFor} onChange={event=>setScheduledFor(event.target.value)} disabled={state==="scheduling"}/></label><button type="button" onClick={schedule} disabled={!scheduledFor||state==="scheduling"||!contacts}><CalendarClock size={16}/>{state==="scheduling"?"Scheduling…":"Schedule WhatsApp"}</button></>}
