...
export default function ContentStudio({initialBrandProfile}:{initialBrandProfile:BrandProfile}) {
  ...
  const [brandProfile,setBrandProfile]=useState(initialBrandProfile),[goal,setGoal]=useState("new-arrival"),[language,setLanguage]=useState("English"),[style,setStyle]=useState("minimal-luxury"),[offer,setOffer]=useState("");
  const [audience,setAudience]=useState(initialBrandProfile.audiences[0]||"Women in the UAE seeking premium, affordable fashion");
  const [status,setStatus]=useState(""),[error,setError]=useState(""),[tab,setTab]=useState("copy"),[background,setBackground]=useState<string>(),[loading,setLoading]=useState(false);
  ...
  async function imp(){setLoading(true);setError("");setStatus("Importing Shopify product...");try{const data=await post("/api/import-product",{url});setProduct(data.product);setCampaign(null);setStatus("Product imported. Review settings, then generate.");}catch(e){setError(e instanceof Error?e.message:"Import failed");}finally{setLoading(false)}}
  ...
  return <div className="shell"><header className="topbar"><div className="brandmark">KAVIA<span>ACTIVEWEAR & LIFESTYLE</span></div><div className="badge">eSoukk AI Content Studio · Approval Mode</div></header><main>
    ...
    <section className="panel form-panel" id="campaign-settings"><div className="field"><label>SHOPIFY PRODUCT LINK</label><input value={url} onChange={e=>setUrl(e.target.value)}/></div><div className="actions"><button type="button" className="primary" onClick={imp} disabled={loading}><ShoppingBag size={17}/> Import Product</button></div>
    ...
    {product&&<section className="workspace"><aside className="panel product-card">{product.images[0]&&<img src={product.images[0]} alt={product.title}/>}<h2>{product.title}</h2><div className="price">{price}</div><p>{product.description.slice(0,260)}{product.description.length>260?"…":""}</p><a href={product.url} target="_blank" rel="noreferrer">Open product ↗</a></aside><div className="panel output-panel">{!campaign?<div className="empty">Generate the campaign to see platform variations, creatives, video guidance and SEO.</div>:<><div className="tabs">{["copy","creatives","video","seo"].map(item=><button type="button" key={item} className={`tab ${tab===item?"active":""}`} onClick={()=>setTab(item)}>{item.toUpperCase()}</button>)}</div>
      {tab==="copy"&&<div className="copy-grid">{copyCards.map(([label,text])=><div className="copy-card platform-copy-card" key={label}><h3>{label}</h3><p>{text}</p><button type="button" onClick={()=>cp(text)}>Copy {label}</button></div>)}</div>}
      {tab==="creatives"&&<div className="creative-grid"><CreativeCanvas label="Instagram Portrait" width={1080} height={1350} product={product} campaign={campaign} background={background}/><CreativeCanvas label="Facebook Square" width={1080} height={1080} product={product} campaign={campaign} background={background}/><CreativeCanvas label="Instagram Story" width={1080} height={1920} product={product} campaign={campaign} background={background}/><CreativeCanvas label="Pinterest Pin" width={1000} height={1500} product={product} campaign={campaign} background={background}/></div>}
