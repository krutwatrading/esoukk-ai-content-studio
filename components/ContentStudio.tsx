...
type ApiResult={product:ProductData;campaign:CampaignCopy;imageUrl:string;error?:string};
...
export default function ContentStudio({initialBrandProfile}:{initialBrandProfile:BrandProfile}) {
  ...
  const [status,setStatus]=useState(""),[error,setError]=useState(""),[tab,setTab]=useState("copy"),[background,setBackground]=useState<string>(),[publishImage,setPublishImage]=useState<string>(),[loading,setLoading]=useState(false),[importing,setImporting]=useState(false),[imported,setImported]=useState(false),[copied,setCopied]=useState<string|null>(null);
  ...
  async function lifestyle(){if(!product)return;setLoading(true);setError("");setStatus("Creating a new model, pose, setting and campaign photograph. This normally takes 30–90 seconds.");try{const data=await post("/api/generate-lifestyle",{productTitle:product.title,productDescription:product.description,productImage:product.images[0],style});setBackground(data.imageUrl);setPublishImage(undefined);setStatus("Your new AI campaign photograph is ready. Compare it with the Shopify reference below.");window.setTimeout(()=>document.getElementById("ai-photo-review")?.scrollIntoView({behavior:"smooth",block:"center"}),100);}catch(e){setStatus("");setError(e instanceof Error?e.message:"Image generation failed");}finally{setLoading(false)}}
  ...
  return <div className="shell"><header className="topbar"><BrandLogo className="header-logo"/><div className="badge">eSoukk AI Content Studio · Approval Mode</div></header><main>
    ...
    <section className="panel form-panel" id="campaign-settings"><div className="field"><label>SHOPIFY PRODUCT LINK</label><input value={url} onChange={e=>{setUrl(e.target.value);setImported(false)}}/></div><div className="actions"><button type="button" className={`primary ${imported?"action-confirmed":""}`} onClick={imp} disabled={loading||importing} aria-busy={importing}><ShoppingBag size={17}/>{importing?"Importing…":imported?"Imported ✓":"Import Product"}</button></div>
      ...
      {product&&background&&<section className="ai-photo-review" id="ai-photo-review"><div className="review-heading"><span>AI PHOTO REVIEW</span><h3>Check product accuracy before creating assets</h3><p>Compare colour, shape, neckline, fabric and construction. Approve only when the generated product matches the Shopify reference.</p></div><div className="review-images"><figure><figcaption>SHOPIFY REFERENCE</figcaption><img src={product.images[0]} alt={`${product.title} Shopify reference`}/></figure><figure><figcaption>NEW AI CAMPAIGN PHOTO</figcaption><img src={background} alt={`${product.title} AI campaign`}/></figure></div><div className="review-actions"><button type="button" className="approve-photo" onClick={approveAiPhoto} disabled={loading}>Approve &amp; Build Creatives</button><button type="button" className="regenerate-photo" onClick={lifestyle} disabled={loading}>Regenerate Photo</button></div></section>}
    </section>
    {product&&<section className="workspace"><aside className="panel product-card">{product.images[0]&&<img src={product.images[0]} alt={product.title}/>}<h2>{product.title}</h2><div className="price">{price}</div><p>{product.description.slice(0,260)}{product.description.length>260?"…":""}</p><a href={product.url} target="_blank" rel="noreferrer">Open product ↗</a></aside><div className="panel output-panel">{!campaign?<div className="empty">Generate the campaign to see platform variations, creatives, video guidance and SEO.</div>:<><div className="tabs">{["copy","creatives","video","seo"].map(item=><button type="button" key={item} className={`tab ${tab===item?"active":""}`} onClick={()=>setTab(item)}>{item.toUpperCase()}</button>)}</div>
      ...
      {tab==="creatives"&&<div className="creative-grid"><CreativeCanvas label="Instagram Portrait" width={1080} height={1350} product={product} campaign={campaign} background={background} style={style as VisualStyle} onSelectForPublishing={dataUrl=>{setPublishImage(dataUrl);setStatus("Instagram Portrait selected for approval and publishing.");setTab("copy")}}/><CreativeCanvas label="Facebook Square" width={1080} height={1080} product={product} campaign={campaign} background={background} style={style as VisualStyle}/><CreativeCanvas label="Instagram Story" width={1080} height={1920} product={product} campaign={campaign} background={background} style={style as VisualStyle}/><CreativeCanvas label="Pinterest Pin" width={1000} height={1500} product={product} campaign={campaign} background={background} style={style as VisualStyle}/></div>}
