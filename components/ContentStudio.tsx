...
export default function ContentStudio({initialBrandProfile}:{initialBrandProfile:BrandProfile}) {
  ...
  async function lifestyle(){if(!product)return;setLoading(true);setError("");setStatus("Creating a new model, pose, setting and campaign photograph. This normally takes 30–90 seconds.");try{const data=await post("/api/generate-lifestyle",{productTitle:product.title,productDescription:product.description,productImage:product.images[0],style});setBackground(data.dataUrl);setPublishImage(undefined);setStatus("New AI campaign photograph generated. Review product accuracy, then select it in Creatives.");setTab("creatives");}catch(e){setStatus("");setError(e instanceof Error?e.message:"Image generation failed");}finally{setLoading(false)}}
