...
export default function ContentStudio({initialBrandProfile}:{initialBrandProfile:BrandProfile}) {
  ...
  async function lifestyle(){if(!product)return;setLoading(true);setError("");setStatus("Generating a completely new model-led campaign photograph…");try{const data=await post("/api/generate-lifestyle",{productTitle:product.title,productDescription:product.description,productImage:product.images[0],style});setBackground(data.dataUrl);setPublishImage(undefined);setStatus("New AI campaign photograph generated. Review product accuracy, then select it in Creatives.");setTab("creatives");}catch(e){setError(e instanceof Error?e.message:"Image generation failed");}finally{setLoading(false)}}
